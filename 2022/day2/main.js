...
const getFirst = () => {
    ...
    const checkResult = (p1, p2) => {
        if ((ROCK.includes(p1) && ROCK.includes(p2)) || (PAPER.includes(p1) && PAPER.includes(p2)) || SCISSORS.includes(p1) && SCISSORS.includes(p2)) {
            return 3
        }
        if (ROCK.includes(p1) && PAPER.includes(p2) || PAPER.includes(p1) && SCISSORS.includes(p2) || SCISSORS.includes(p1) && ROCK.includes(p2)) {
            return 6
        }
        if (ROCK.includes(p1) && SCISSORS.includes(p2) || PAPER.includes(p1) && ROCK.includes(p2) || SCISSORS.includes(p1) && PAPER.includes(p2)) {
            return 0
        }
        throw new Error('WOW')
    }
