...
const findWinnerBoard = (row, boardArray) => {
  ...
  for (let rowItem of row) {
    ...
    for (let board of boardArray) {
      ...
      const isBoardHasMatchedRow = board.some((row) =>
        row.every((item) => steps.includes(item))
      )
      const isBoardHasMatchedColumn = boardColumns.some((col) =>
        col.every((item) => steps.includes(item))
      )
...
const findLastWinningBoard = (row, boards) => {
  ...
  for (let rowItem of row) {
    ...
    for (let board of boardArray) {
      ...
      if (isBoardHasMatchedColumn || isBoardHasMatchedRow) {
        const [winningBoard] = boardArray.splice(
          boardArray.findIndex((el) => el === board),
          1
        )
...
const getSumOfUnmarkedNumbers = (steps, board) =>
  board
    .flat()
    .filter((el) => !steps.includes(el))
