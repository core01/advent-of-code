...
const getFirst = () => {
  ...
  for (let i = 0; i < rows.length; i++) {
    for (let j = 0; j < rows[i].length; j++) {
      const { left, top, right, bottom, center } = getMatrixAdjacentLocations(
        rows,
        i,
        j
      )
...
const findAllMoreValues = (rows, i, j, compareValue) => {
  ...
  for (let [key, location] of [
    [left, leftCoordinates],
    [right, rightCoordinates],
    [top, topCoordinates],
    [bottom, bottomCoordinates],
  ]) {
    if (
      key !== Infinity &&
      key !== 9 &&
      ...
      if (key > compareValue) {
        ...
        coordinates.push(
          location,
          ...findAllMoreValues(rows, location[0], location[1], key)
        )
      }
    }
  }
  ...
  for (let [key, location] of [
    ...
    if (key !== Infinity && key !== 9) {
      if (
        ...
        coordinates.push(
          location,
          ...findAllLessValues(rows, location[0], location[1], key)
        )
      }
...
const getSecond = () => {
  ...
  for (let i = 0; i < rows.length; i++) {
    for (let j = 0; j < rows[i].length; j++) {
      const { center, centerCoordinates } = getMatrixAdjacentLocations(
        rows,
        i,
        j
      )
      ...
      if (
        center === 9 ||
        visitedCoordinates.some((el) => el[0] === i && el[1] === j)
      )
        continue

      const moreValues = findAllMoreValues(
        rows,
        centerCoordinates[0],
        centerCoordinates[1],
        center
      )
      const minValues = findAllLessValues(
        rows,
        centerCoordinates[0],
        centerCoordinates[1],
        center
  ...
  basin.forEach((el) => {
    let candidate = []
    el.forEach((e) => {
      if (!candidate.some((can) => can[0] === e[0] && can[1] === e[1]))
        candidate.push(e)
    })
