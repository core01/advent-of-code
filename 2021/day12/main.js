...
const findAllPaths = (
  pathObject,
  pathArray = ['start'],
  allowVisitSmallCaveTwice = false
) => {
  let foundPaths = []
  const from = pathArray[pathArray.length - 1]

  for (const to of pathObject[from]) {
    const lowerCaseMembers = pathArray.filter(
      (el) => isLowerCase(el) && el !== 'start' && el !== 'end'
    )
    ...
    let shouldProceedWithCave =
      (allowVisitSmallCaveTwice &&
        !(
          isLowerCase(to) &&
          hasSomeLowerCase2Times &&
          pathArray.filter((el) => el === to).length > 0
        )) ||
      (!allowVisitSmallCaveTwice &&
        !(isLowerCase(to) && pathArray.includes(to)))
    ...
    if (shouldProceedWithCave) {
      ...
      if (to === 'end') {
        ...
        foundPaths.push(
          ...findAllPaths(pathObject, newPath, allowVisitSmallCaveTwice)
        )
      }
    }
  }
