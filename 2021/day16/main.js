...
const parsePackets = (row, subPackets = -1) => {
  ...
  while (row.length > 0 && (subPackets === -1 || packetsFound < subPackets)) {
    ...
    switch (packet.type) {
      case 0:
        packet.literalValue = packet.subPackets.reduce(
          (prev, cur) => cur.literalValue + prev,
          0
        )
        break
      case 1:
      ...
      case 2:
        packet.literalValue = Math.min(
          ...packet.subPackets.map((packet) => packet.literalValue)
        )
        break
      case 3:
      ...
      case 5:
        packet.literalValue =
          packet.subPackets[0].literalValue > packet.subPackets[1].literalValue
            ? 1
            : 0
        break
      case 6:
      ...
      case 7:
        packet.literalValue =
          packet.subPackets[0].literalValue ===
          packet.subPackets[1].literalValue
            ? 1
            : 0
        break
    }
