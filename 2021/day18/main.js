...
for (let i = 0; i < rows.length; i += 2) {
    ...
    if (explodeIndex > -1) {
        let isHasLeftRegularNumber = explodeIndex > 0 && newRow[explodeIndex - 1].depth === newRow[explodeIndex].depth - 1;
        let isHasRightRegularNumber = newRow[explodeIndex + 2].depth === newRow[explodeIndex + 1].depth - 1;
        if (isHasLeftRegularNumber) {
            newRow[explodeIndex - 1].value = newRow[explodeIndex - 1].value + newRow[explodeIndex].value
        }
        if (isHasRightRegularNumber) {
            newRow[explodeIndex + 2].value = newRow[explodeIndex + 2].value + newRow[explodeIndex + 1].value
        }
