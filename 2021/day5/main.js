...
const prepareData = (inputData) => {
    let maxValue = 0
    const data = inputData.map(el => el.split(' -> ').map(el => el.split(',').map(el => parseInt(el, 10))))
...
const processResult = (processLinearOnly = false) => {
    ...
    let data = processLinearOnly ? preparedData.filter(el => isLinear(...getCoordinates(el))) : preparedData
    ...
    data.forEach(el => {
        ...
        arrayOfCoordinates.forEach(({x, y}) => {
            ...
            if (matrix[x][y] > 1 && !visitedCoordinates.some(visited => (visited.x === x && visited.y === y))) {
